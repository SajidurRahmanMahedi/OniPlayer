...
│   │       ├── java
│   │       │   └── com/media/oniplayer
│   │       │       ├── FolderAdapter.kt   <- Adapter for Folder List with batch selection UI
│   │       │       ├── FolderItem.kt      <- Data Model for folders
│   │       │       ├── MainActivity.kt    <- App entrypoint, Core Controller & Gesture engine
│   │       │       ├── VideoAdapter.kt    <- Adapter for Videos with thumbnails & progress bars
│   │       │       └── VideoItem.kt       <- Data Model for video files
│   │       └── res
│   │           ├── layout
│   │           │   ├── activity_main.xml            <- Layout housing library lists & Player components
│   │           │   ├── dialog_delete_confirm.xml    <- Modern rounded alert dialog for batch delete
│   │           │   └── dialog_track_selection.xml   <- Elegant BottomSheet for Audio/Subtitle tracks
│   │           └── values
│   │               └── colors.xml                   <- Sleek, high-contrast dark theme resources
└── settings.gradle.kts
```
...
3.  Let Gradle sync finish. The project is pre-configured to fetch the VLC Maven artifacts:
    ...
    dependencyResolutionManagement {
        repositories {
            google()
            mavenCentral()
            maven { url = uri("https://vlc-android.videolan.org/maven2/") }
        }
    }
    ```
...
*   [MainActivity.kt](file:///c:/Users/Archer/Desktop/OniPlayer-Android-main/app/src/main/java/com/media/oniplayer/MainActivity.kt) — Houses the LibVLC lifecycle, complex event routing, gesture indicators (scrubbing, volume, brightness, zoom), and state transitions.
*   [VideoAdapter.kt](file:///c:/Users/Archer/Desktop/OniPlayer-Android-main/app/src/main/java/com/media/oniplayer/VideoAdapter.kt) — Manages thumbnail loading via Glide and computes remaining playback progress percentages.
*   [FolderAdapter.kt](file:///c:/Users/Archer/Desktop/OniPlayer-Android-main/app/src/main/java/com/media/oniplayer/FolderAdapter.kt) — Binds directory hierarchies with dynamic count trackers and selection highlights.
*   [colors.xml](file:///c:/Users/Archer/Desktop/OniPlayer-Android-main/app/src/main/res/values/colors.xml) — Defines the core UI palette designed to give OniPlayer its distinct dark look.
